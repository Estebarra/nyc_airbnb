...
def delta_date_feature(dates):
    ...
    Given a 2d array containing dates (in any format recognized by pd.to_datetime),
     it returns the delta in days between each date and the most recent date in
    its column
    """
...
def go(args):
    """
    Main function to train a Random Forest
    """
...
def plot_feature_importance(pipe, feat_names):
    """
    Create a bar plot to show the impact of each variable
    """
...
def get_inference_pipeline(rf_config, max_tfidf_features):
    """
    Construction of the evaluation pipeline
    """
